...
def main():
    # Parsing
    parser = argparse.ArgumentParser(description="Run final hyperparameter selection for a single hyperparameter combination (array job).")
    ...
    arg_configs = {
        ...
        'k_folds': {
            'type': int,
            'default': 5,
            'help': 'Number of folds for train/test splits (default: 5)'
        },
        'n_max_param': {
            'type': int,
            'default': 96,
        ...
        'run_name': {
            'type': str,
            'default': 'final',
            'help': 'Name of the run (default: final)'
        }
    }
    ...
    print(f"Using {k_folds} folds for train/test splits")
    ...
    # Start the final hyperparameter selection process for this single parameter combination
    print("Starting final hyperparameter selection process for single parameter combination.")
    ...
    if args.fold_type == "CV":
        for multi_type in multi_types:
            ...
            output_filename = f"{args.model_type}_final_selection_{multi_type}_param_{param_index:03d}_{time}.csv"
            df.to_csv(f"{output_dir}/{output_filename}")
            print(f"Saved results to {output_filename}")
            
    elif args.fold_type == "loso":
        for multi_type in multi_types:
            # Use the new LOSO train/test function
            df = train_test.run_train_test_loso_single_param(
                X, y, study_labels, model, single_param, pipe, 
            ...
            # Save results with parameter index in filename
            output_filename = f"{args.model_type}_final_selection_loso_{multi_type}_param_{param_index:03d}_{time}.csv"
            df.to_csv(f"{output_dir}/{output_filename}")
            print(f"Saved results to {output_filename}")
    else:
        raise ValueError(f"Fold type {args.fold_type} not supported.")

    print("Final hyperparameter selection process finished for parameter combination.")
