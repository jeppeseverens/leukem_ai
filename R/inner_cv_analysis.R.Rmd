...
MODEL_CONFIGS <- list(
  svm = list(
    ...
    file_paths = list(
      cv = "/Users/jsevere2/Documents/SVM_array/cv_20aug25_all/",
      loso = "/Users/jsevere2/Documents/SVM_array/loso_20aug25_all/"
    ),
    output_dir = "/Users/jsevere2/Documents/AML_PhD/leukem_ai/inner_cv_best_params_n10/SVM_20aug25"
  ),
  xgboost = list(
    classification_type = "OvR",
    file_paths = list(
      cv = "/Users/jsevere2/Documents/XGBOOST_array/cv_20aug25_all/",
      loso = "/Users/jsevere2/Documents/XGBOOST_array/loso_20aug25_all/"
    ),
    output_dir = "/Users/jsevere2/Documents/AML_PhD/leukem_ai/inner_cv_best_params_n10/XGBOOST_20aug25"
  ),
  neural_net = list(
    classification_type = "standard",
    file_paths = list(
      cv = "/Users/jsevere2/Documents/NN_array/cv_20aug25_all/",
      loso = "/Users/jsevere2/Documents/NN_array/loso_20aug25_all/"
    ),
    output_dir = "/Users/jsevere2/Documents/AML_PhD/leukem_ai/inner_cv_best_params_n10/NN_20aug25"
  )
)
...
test <- model_results[["neural_net"]][["loso"]] %>% group_by(across(c("inner_fold","outer_fold", "params"))) %>%
    ...
    filter(mean_kappa == max(mean_kappa, na.rm = TRUE)) %>% slice(1)
```
...
for (model_name in names(model_results)) {
    ...
    probability_matrices_2[[model_name]] <- list()
    ...
    for (fold_type in names(model_results[[model_name]])) {
      ...
      if (!is.null(results) && !is.null(best_params)) {
        ...
        probability_matrices_2[[model_name]][[fold_type]] <- probs
        remove(probs)
      }
    }
}
...
per_model_results <- get_per_model_performance(probability_matrices_2)
```

```{r}
per_model_results <- per_model_results[per_model_results$method == "loso",]
...
keep <- paste0("Class: ", make.names(names(which(table(leukemia_subtypes)<20))))
per_class_results <- per_class_results[per_class_results$class %in% keep,]
per_class_results <- per_class_results %>% group_by(model) %>% summarise(mean_f1 = mean(F1, na.rm = TRUE))
...
ensemble_results <- run_ensemble_analysis_for_both_types(probability_matrices,  generate_weights(0.1))
...
performance_comparisons <- compare_ensemble_performance_for_both_types(
    list(probability_matrices = probability_matrices,  cv = ensemble_results_2$cv, loso = ensemble_results_2$loso)
  )
```
...
rejection_results <- run_complete_rejection_analysis(
    probability_matrices,  ensemble_results_2
  )
```
...
saveRDS(inner_cv_results, "../inner_cv_best_params_n10/inner_cv_results.rds")
```
