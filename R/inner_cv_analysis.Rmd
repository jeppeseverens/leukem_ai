...
label_mapping <- read.csv("~/Documents/AML_PhD/leukem_ai/label_mapping_df_n10.csv")
...
leukemia_subtypes <- read.csv("../data/rgas.csv")$ICC_Subtype[filter]

# Load study metadata
study_names <- read.csv("../data/meta.csv")$Studies[filter]
```
  ...
  for (model_name in names(model_configs)) {
    ...
    for (fold_type in names(config$file_paths)) {
      ...
      if (model_name == "neural_net") {
        ...
        results <- data.frame(data.table::fread(file_path))
      }
...
library(dplyr)
get_per_model_performance(probability_matrices) %>% group_by(model, method) %>% summarise(mean_kappa = mean(kappa), 
                                                                           sd_kappa = sd(kappa),
                                                                           mean_mcc = mean(mcc),
                                                                           sd_mcc = sd(mcc))
...
```{r}
generate_weights <- function(step = 0.1){
