...
# inner_cv_results_unmerged$final_results
# inner_cv_results_merged$final_results
# inner_cv_results_mds_only$final_results
```
...
# outer_cv_results_mds_only[["performance_summaries"]]
```

```{r}
outer_unmerged <-  readRDS("../data/out/outer_cv/outer_cv_results_28jan2025_nomdspaed_unmerged_maxprob.rds")
outer_merged <- readRDS("../data/out/outer_cv/outer_cv_results_28jan2025_nomdspaed_merged_maxprob.rds")
```
...
f1_comparison <- f1_comparison %>%
  mutate(
    Category = case_when(
      Class %in% c("PML::RARA", "RUNX1::RUNX1T1", "CBFB::MYH11", "DEK::NUP214", "MECOM rearrangement", "MLLT3::KMT2A") ~ "Major rearrangements",
...
axis_class_levels <- levels(f1_comparison$Class)
# Filter to only classes present in the data
axis_class_levels <- axis_class_levels[axis_class_levels %in% unique(f1_comparison$Class)]

# Create axis text colors based on category (MDS-related and KMT2A get their group colors)
...
axis_labels <- sapply(axis_class_levels, function(cls) {
  if (cls %in% c("Other KMT2A rearrangements", "MDS-related")) {
    paste0("**", cls, "**")
  } else {
    cls
  }
...
# Panel C: KMT2A::MLLT3 separate, other KMT2A combined, MDS combined
kmt2a_other_merged <- "Other KMT2A rearrangements"
mds_merged <- "MDS-related"

# Combine groups for each panel
focus_groups_panel_b <- c(kmt2a_mllt3, kmt2a_other_full, mds_groups_full)
focus_groups_panel_c <- c(kmt2a_mllt3, kmt2a_other_merged, mds_merged)
...
create_error_heatmap <- function(error_data, title, other_kmt2a_classes, mds_classes) {
  ...
  # Create annotation for groups: MLLT3::KMT2A, Other KMT2A, MDS-related
  ...
  # Set factor order for consistent display
  group_anno <- factor(group_anno, levels = c("KMT2A::MLLT3", "Other KMT2A", "MDS-related"))
  
  ha <- rowAnnotation(
    Group = group_anno,
    col = list(Group = c("KMT2A::MLLT3" = "#1f77b4", "Other KMT2A" = "#ff7f0e", "MDS-related" = "#d62728")),
...
# Create Panel B: Full classes (MLLT3::KMT2A separate, other KMT2A separate, MDS separate)
...
# Create Panel C: Collapsed classes (MLLT3::KMT2A separate, Other KMT2A combined, MDS combined)
create_error_heatmap(
  error_panel_c, 
...
loso_performance <- outer_merged$detailed_performance$loso$svm
...
for (fold in names(outer_merged$detailed_performance$loso$svm)) {
  fold_prob_matrix <- outer_merged$outer_probability_matrices$svm$loso[[fold]]
...
# Helper function to get per-class F1 per fold for LOSO
get_f1_per_fold_loso <- function(res_obj, label_set_name) {
  ...
  for (fold in folds) {
    ...
    fold_perf <- res_obj$detailed_performance$loso$svm[[fold]]
...
p_f1_loso_facet <- ggplot(f1_loso_plot, aes(x = f1, y = class, color = fold)) +
  ...
  labs(
    ...
    subtitle = "Each dot represents performance on a LOSO outer fold",
    x = "F1 Score",
    y = "AML Subtype"
  ) +
  theme(
...
# Focus on MDS-related and other KMT2A classes that were merged
...
# Calculate mean F1 per fold for full subclasses and collapsed classes
delta_f1_list <- list()

for (collapsed_class in names(merge_mapping)) {
  full_classes <- merge_mapping[[collapsed_class]]
  
  # Get F1 for the collapsed class per fold (from merged results)
  collapsed_f1 <- f1_merged_loso %>%
    filter(class == collapsed_class) %>%
...
p_delta_f1 <- ggplot(delta_f1_data, aes(x = group, y = delta_f1)) +
  ...
  labs(
    ...
    subtitle = sprintf("MDS-related: mean dF1 = +%.2f | Other KMT2A: mean dF1 = +%.2f",
                       delta_summary$mean_delta[delta_summary$group == "MDS-related"],
                       delta_summary$mean_delta[delta_summary$group == "Other KMT2A rearrangements"]),
...
df_global <- read.csv("../data/out/inner_cv/ensemble_weights_mds_only_maxprob/cv/")
