...
all_classes_ordered <- c(
  ...
  "MLLT3::KMT2A", "Other KMT2A rearrangements", "KMT2A::ELL", "KMT2A::MLLT1", "KMT2A::MLLT10", "KMT2A::MLLT4", "KMT2A::SEPT6",
...
make_faceted_cm <- function(type) {
  ...
  conf_matrices <- list()
  for (fold in folds) {
    per_class <- outer_unmerged$detailed_performance[[type]]$OvR_Ensemble[[fold]]$confusion_matrix$byClass
    rownames(per_class) <- gsub("Class: ", "", rownames(per_class))
    rownames(per_class) <- fix_names(rownames(per_class))
    
    cm <- outer_unmerged$detailed_performance[[type]]$OvR_Ensemble[[fold]]$confusion_matrix$table
